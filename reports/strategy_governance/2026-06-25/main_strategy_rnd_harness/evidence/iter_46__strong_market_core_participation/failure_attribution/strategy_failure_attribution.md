...
- overfit: `/home/zj/workspace/stok-mapping/reports/strategy_governance/2026-06-25/main_strategy_rnd_harness/evidence/iter_46__strong_market_core_participation/admission/overfit_diagnostic/strategy_overfit_diagnostic.csv`

## 汇总

| strategy_id | preset | action | severity | primary_failure | return | execution | construction | factor | parameter | regime | data |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| strong_market_core_participation_v1 | baseline_2y_1y_5fold | reject | 高 | 收益失败 | high | high | high | none | none | high | none |
...
- 证据：收益失败：年化收益均值 -0.0330 未通过 gate 0.0000；Sharpe 均值 -0.4277 未通过 gate 0.5000；正收益折比例 0.00% 未通过 gate 75.00% | 执行失败：换手未通过 admission gate：mean=4.08, max=16.04 | 组合构造失败：行业集中度触发 admission 审计阈值：top1_mean=6.21%, top3_mean=12.86%, violation_days=68 | 市场阶段失败：策略级 overfit_risk_level=high
