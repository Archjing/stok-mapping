...
def _module_from_filename(filename: str) -> str:
    ...
    if classify_legacy_brief_artifact(stem) is not None:
        return "brief"
...
LEGACY_WATCHLIST_PREFIX = "phase0_watchlist"
LEGACY_PREMARKET_PREFIX = "phase0_premarket"


def classify_legacy_brief_artifact(filename: str) -> str | None:
    """Classify a legacy ``phase0_*`` brief artifact, or ``None`` for other names."""
    stem = Path(filename).stem
    if stem.startswith(LEGACY_WATCHLIST_PREFIX) or stem.startswith(LEGACY_PREMARKET_PREFIX):
        return "brief"
    return None
