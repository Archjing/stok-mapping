...
from quant.intelligence.candidates import write_candidates
from quant.intelligence.common import configured_path, date_tag, resolve_path, safe_text
from quant.intelligence.schema import IntelligenceResult
...
def _dedupe_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for row in rows:
        key = (
            ...
            safe_text(row.get("source_path_or_url")),
    ...
    if suffix == ".md":
        ...
        try:
            ...
            for _, item in frame.iterrows():
                title = safe_text(item.get(title_col)) if title_col else ""
                if not title:
                    continue
                rows.append(
                    _candidate_row(
                        title=title,
                        source_type="paper",
                        source_path_or_url=safe_text(item.get(url_col)) if url_col else rel,
                        published_at=safe_text(item.get(date_col)) if date_col else "",
                        market_scope=safe_text(item.get("market_scope")),
            ...
            for item in items:
                ...
                rows.append(
                    _candidate_row(
                        title=title,
                        source_type="paper",
                        source_path_or_url=safe_text(item.get("paper_url") or item.get("url") or rel),
                        published_at=safe_text(item.get("year") or item.get("published_at")),
...
def _collect_local_dir(root: Path, raw_source: str | Path, limit: int | None) -> tuple[list[dict[str, str]], list[str]]:
    source = resolve_path(root, raw_source)
    warnings: list[str] = []
    rows: list[dict[str, str]] = []
...
def import_local_intelligence(
    ...
    *,
    ...
    output = configured_path(
        ...
        fallback=f"{inbox_dir}/intelligence_candidates_{date_tag()}.csv",
    )
    report = configured_path(
        ...
        fallback=report_config_path(root=root, config=cfg, value=f"{report_dir}/intelligence_import_local_report_{date_tag()}.md"),
    )
...
def _fetch_rss(url: str, limit: int | None) -> tuple[list[dict[str, str]], str | None]:
    ...
    for entry in entries:
        ...
        published = safe_text(entry.get("published") or entry.get("updated"))
        if title:
            rows.append(
                _candidate_row(
                    title=title,
    ...
    for entry in parsed.entries:
        rows.append(
            _candidate_row(
                title=safe_text(entry.get("title")),
                source_type="paper",
                source_path_or_url=safe_text(entry.get("link")),
                published_at=safe_text(entry.get("published"))[:10],
    ...
    for item in data.get("results", []):
        ...
        rows.append(
            _candidate_row(
                title=title,
                source_type="paper",
                source_path_or_url=safe_text(item.get("doi") or item.get("id")),
                published_at=safe_text(item.get("publication_year")),
...
def _fetch_crossref(query: str, limit: int | None) -> tuple[list[dict[str, str]], str | None]:
    ...
    for item in data.get("message", {}).get("items", []):
        ...
        title = safe_text(titles[0] if titles else "")
        if not title:
            continue
        year = ""
        ...
        rows.append(
            _candidate_row(
                title=title,
                source_type="paper",
                source_path_or_url=safe_text(item.get("URL") or item.get("DOI")),
                published_at=year,
...
def _fetch_sia_sales(source: dict[str, Any], root: Path, limit: int | None) -> tuple[list[dict[str, str]], str | None]:
    ...
    """
    try:
        from quant.ai_corpus.providers.sia_sales import fetch_sia_sales
...
def collect_intelligence(
    ...
    *,
    ...
    report = configured_path(
        ...
        fallback=report_config_path(root=root, config=cfg, value=f"{report_dir}/intelligence_collect_report_{date_tag()}.md"),
    )
    rows: list[dict[str, str]] = []
    warnings: list[str] = []
    source_counts: dict[str, int] = {}
