...
4. **盘前交付**：复用并实时刷新现有观察池/简报/对照图页面，取代"每晚 build + rsync"的静态快照。
5. **系统观测**：调度任务状态、数据健康、回填审计一览。
...
> 前端全部构建为静态资源，由 FastAPI 在 `/` 提供（生产），开发期 Vite dev server 代理到 `127.0.0.1:8000`。单一仓库（monorepo 单目录）结构，见 §6。
...
- 打开 `http://127.0.0.1:8000` 能看到与现有静态站点视觉一致的账户总览、账户详情、观察池、对照图。
