...
工程目标：把“策略想法 -> 数据准备 -> smoke test -> walk-forward -> gate -> 报告归档 -> 市场环境归因 -> 策略选择方法论更新”固化成统一开发标准，减少拍脑袋试错、口径漂移和无效回测。
...
- 策略池入池、降级、停用和方法论复盘；
- 不适用于自动交易执行，不替代账户级实盘风控，也不承诺未来收益。
...
策略池扩张必须服务于策略选择方法论，而不是为了让候选数量变多。

每个候选至少要回答：

- 它在哪类市场环境中相对有优势；
- 它在哪类市场风格中容易失效；
- 它相对沪深300、策略基准或同类候选的优势来自哪里；
- 当前应当启用、降权、停用还是仅观察；
- 下一次复盘时用什么证据更新这个判断。
...
3. 它改善的目标是什么：当前市场下的实盘决策辅助、年化、Sharpe、回撤、换手、解释性，还是账户执行现实性？
4. 它最终输出的是什么：排名、候选池、权重、风控缩放还是解释标签？
5. 它为什么在 A 股市场里应该有效？
6. 它适合什么市场环境和市场风格，不适合什么环境和风格？
7. 它对策略选择方法论的增量贡献是什么？

如果第 2、5、6、7 条答不清，就不进入开发。
