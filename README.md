...
Walk-forward 加速采用保守缓存边界：同一数据源签名下，admission 多 preset 不再反复清空 symbol 级行情 LRU；同一股票池、同一日期窗口、同一 as-of 口径和同一数据源签名下的 fold panel 可以复用；策略自己的 `prepare_panel` 只按 `strategy_id + strategy_cfg + fold 输入指纹` 缓存，不跨策略共享。诊断耗时时可运行：
