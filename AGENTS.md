...
- `main` 分支只承载全局代码、配置、目录规则和解释这些规则的文档。研究分支默认只允许生成、移动或整理该研究分支自己的产物；不得在研究分支直接修改 `phase0/`、`scripts/`、`tests/`、`config.yaml` 等源代码或主配置。若研究过程中发现必须改代码，应先停止研究分支操作，把需求提升为独立 `main` 任务，在 `main` 或专门的代码集成 worktree 完成、验证、提交后，再让研究分支同步 `main`。
